...
class ORFFinder:
    ...
    '''
    def __init__(self, file, args, inHeader = "", inSequence=""):
        '''
        Initialize ORF finder by saving user input parameters and saving the sequence to process.
        Call the methods within ORFFinderthat will do the task of parsing the sequence for ORFs
        '''
        # in more advanced scenario, user can determine minimum ORF length
        ...
        reverseComp = Seq(inSequence)
        self.findGenes(reverseComp.reverse_complement())
...
def main(inCL=None):
    ...
    print(myCMD.args)
    myReader = FastAReader(myCMD.args.inFile)
    file = open(myCMD.args.outFile, "w")
    for head, seq in myReader.readFasta():
        newORFs = ORFFinder( file, myCMD.args, head, seq)
    file.close()
